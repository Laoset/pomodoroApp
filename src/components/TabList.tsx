    ...
    <div className="flex flex-row justify-between items-center gap-4 mb-4 w-full border-gray-300 bg-white/80 backdrop-blur-sm">
      {elementsTab.map((element) => (
        <button
          key={element.id}
          className={
            "flex items-center text-center px-2 py-1 transition-colors cursor-pointer w-full justify-center gap-2 rounded-sm text-xl" +
            (section === element.id
        ...
        >
          <span className="w-4 h-4">{element.icon}</span>
          {element.name}
        </button>
      ))}
    </div>
